...
def parse_ip_country_data():
    ...
    with zipfile.ZipFile(file_path, "r") as zip_file_handle:
        license_file_name, readme_file_name, csv_file_name = (
        ...
        if EXPECTED_COPYRIGHT_AUTHOR not in copyright_notice:
            raise ValueError(
                f"Missing author '{EXPECTED_COPYRIGHT_AUTHOR}' in"
                f" copyright notice '{copyright_notice}'"
            )
        ...
        with zip_file_handle.open(csv_file_name) as csv_file_handle:
            for row in csv.reader(io.TextIOWrapper(csv_file_handle)):
                ...
                if (
                    len(country_code) != 2 or not country_code.isupper()
                ) and country_code != "-":
...
def update_ip_country_data():
    ...
    h_timestamp_updated = time.strftime(
        "%Y-%m-%d", time.localtime(timestamp_updated)
    )
    country_codes, ip_ranges, country_values, copyright_notice = (
        parse_ip_country_data()
    )
...
""".encode())
    ...
    for country_code in country_values:
        output_country_codes.extend(
            f'{country_code.replace("-", "_")},'.encode()
        )

...
class Plugin(BasePlugin):
    ...
    def user_stats_notification(self, user, stats):
        ...
        if user in (i[0] for i in self.config.sections["server"]["userlist"]):
            self.log(
                "Buddy %s is only sharing %s files in %s folders. Not complaining.",
                (user, stats["files"], stats["dirs"]),
            )
        ...
        if stats["files"] == 0 and stats["dirs"] == 0:
            ...
            self.log(
                "User %s seems to have zero files and no public shared folder, the server could be wrong.",
                user,
            )

        self.log(
            "Leecher detected, %s is only sharing %s files in %s folders. "
            + "Going to "
            + self.str_action
            + " after transfer…",
            (user, stats["files"], stats["dirs"]),
    ...
    def upload_finished_notification(self, user, *_):
        ...
        if not self.settings["message"]:
            self.log(
                "Leecher %s doesn't share enough files. No message is specified in plugin settings.",
                user,
            )
            return

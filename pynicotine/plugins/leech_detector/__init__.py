...
class Plugin(BasePlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.settings = {
            "message": (
                "Please consider sharing more files if you would like"
                " to download from me again. Thanks :)"
            ),
        ...
        self.metasettings = {
            "message": {
                "description": (
                    "Private chat message to send to leechers. Each"
                    " line is sent as a separate message, too many"
                    " message lines may get you temporarily banned for"
                    " spam!"
                ),
                "type": "textview",
            },
            "open_private_chat": {
                "description": (
                    "Open chat tabs when sending private messages to leechers"
                ),
                "type": "bool",
            },
            "num_files": {
                "description": (
                    "Require users to have a minimum number of shared files:"
                ),
                "type": "int",
                "minimum": 0,
            },
            "num_folders": {
                "description": (
                    "Require users to have a minimum number of shared folders:"
                ),
                "type": "int",
                "minimum": 1,
            },
    ...
    def loaded_notification(self):
        ...
        self.log(
            "Require users have a minimum of %d files in %d shared public"
            " folders.",
    ...
    def check_user(self, user, num_files, num_folders, source="server"):
        ...
        if self.probed_users[user] == "requesting_shares" and source != "peer":
            # Waiting for stats from peer, but received stats from server.
            # Ignore.
            return
        ...
        if is_user_accepted or user in self.core.buddies.users:
            ...
            if is_user_accepted:
                self.log(
                    "User %s is okay, sharing %s files in %s folders.",
                    (user, num_files, num_folders),
                )
            else:
                self.log(
                    "Buddy %s is sharing %s files in %s folders. Not"
                    " complaining.",
                    (user, num_files, num_folders),
                )
            return
        ...
        if (num_files <= 0 or num_folders <= 0) and self.probed_users[
            user
        ] != "requesting_shares":
            # SoulseekQt only sends the number of shared files/folders to the
            # server once on startup. Verify user's actual number of
            # files/folders.
            self.log(
                "User %s has no shared files according to the server,"
                " requesting shares to verify…",
                user,
            )
        ...
        if self.settings["message"]:
            log_message = (
                "Leecher detected, %s is only sharing %s files in %s"
                " folders. Going to message leecher after transfer…"
            )
        else:
            log_message = (
                "Leecher detected, %s is only sharing %s files in %s"
                " folders. Going to log leecher after transfer…"
            )
    ...
    def upload_queued_notification(self, user, virtual_path, real_path):
        ...
        if stats.files is not None and stats.folders is not None:
            self.check_user(
                user, num_files=stats.files, num_folders=stats.folders
            )
    ...
    def upload_finished_notification(self, user, *_):
        ...
        if not self.settings["message"]:
            self.log(
                "Leecher %s doesn't share enough files. No message is"
                " specified in plugin settings.",
                user,
            )
            return
        ...
        for line in self.settings["message"].splitlines():
            for placeholder, option_key in self.PLACEHOLDERS.items():
                # Replace message placeholders with actual values specified

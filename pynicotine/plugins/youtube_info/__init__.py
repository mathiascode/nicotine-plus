...
class Plugin(BasePlugin):
    def __init__(self, *args, **kwargs):
        ...
        self.metasettings = {
            "api_key": {"description": "YouTube Data v3 API key:", "type": "string"},
            "color": {
                "description": "Message color:",
                "type": "dropdown",
                "options": ("Remote", "Local", "Action", "Hilite"),
            },
            "format": {"description": "Message format", "type": "list string"},
        }
    ...
    def get_video_id(self, mode, source, line):
        ...
        match = re.search(
            r"(https?://((m|music)\.)?|www\.)youtu(\.be/|be(-nocookie)?\.com/(embed/|watch\S+v=))"
            r"(?P<video_id>[-\w]{11})",
            line,
        )
        if not match:

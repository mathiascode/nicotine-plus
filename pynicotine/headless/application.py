...
class Application:
    ...
    def on_confirm_quit(self, _only_on_active_uploads):
        responses = "[y/N] "
        cli.prompt(
            _("Do you really want to exit? %s") % responses,
    ...
    def on_invalid_password(self):
        ...
        log.add(
            _(
                "User %s already exists, and the password you entered is"
                " invalid."
            ),
            config.sections["server"]["login"],
        )
        log.add(
            _("Type %s to log in with another username or password."),
            "/connect",
        )
    ...
    def on_setup_username_response(self, user_input):
        ...
        cli.prompt(
            _("Password: "),
            callback=self.on_setup_password_response,
            is_silent=True,
        )
    ...
    def on_setup(self):
        log.add(
            _(
                "To create a new Soulseek account, fill in your desired"
                " username and password. If you already have an"
                " account, fill in your existing login details."
            )
        )
    ...
    def on_shares_unavailable(self, shares):
        ...
        message += "\n" + _(
            "Verify that external disks are mounted and folder permissions are"
            " correct."
        )

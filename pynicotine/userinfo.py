...
class UserInfo:
    ...
    def _get_user_info_response(
        self, requesting_username=None, requesting_ip_address=None
    ):

        if (
            requesting_username is not None
            and requesting_ip_address is not None
        ):
            permission_level, reject_reason = (
                core.shares.check_user_permission(
        ...
        if permission_level == PermissionLevel.BANNED:
            ...
            if reject_reason:
                descr = (
                    "You are not allowed to download my shared"
            ...
            try:
                with open(
                    encode_path(
                        os.path.expandvars(config.sections["userinfo"]["pic"])
                    ),
                    "rb",
                ) as file_handle:
    ...
    def save_user_picture(file_path, picture_bytes):
        ...
        try:
            ...
            log.add(
                _("Cannot save picture to %(filename)s: %(error)s"),
                {"filename": file_path, "error": error},
            )

    def _user_info_progress(
    ...
    def _user_info_request(self, msg):
        ...
        if (
            username in self.requested_info_times
            and request_time < self.requested_info_times[username] + 0.4
        ):
        ...
        log.add(
            _("User %(user)s is viewing your profile"),
            {"user": username},
        )

...
class StartupTest(TestCase):
    ...
    def test_gui_startup(self):
        ...
        if sys.platform not in {"darwin", "win32"}:
            # Display server is required, use GDK's Broadway backend if
            # available. If not available, leave it up to the user to run
            # the tests with e.g. xvfb-run.

            # pylint: disable=consider-using-with
            try:
                broadway_process = subprocess.Popen(
    ...
    def test_cli(self):
        ...
        output = subprocess.check_output(
            [sys.executable, "-m", "pynicotine", "--help"], timeout=3
        )
        self.assertIn(b"--help", output)

        # Check for " 0 folders found after rescan" in output. Text strings
        # are translatable, so we can't match them directly.
        output = subprocess.check_output(
            [
                sys.executable,
                "-m",
        ...
        output = subprocess.check_output(
            [sys.executable, "-m", "pynicotine", "--version"], timeout=3
        )

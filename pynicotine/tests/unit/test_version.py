...
class VersionTest(TestCase):
    ...
    def test_dev_version(self):
        ...
        # Test a sample dev version to ensure it's older than the stable
        # counterpart
    ...
    def test_update_check(self):
        ...
        local_version = UpdateChecker.create_integer_version(
            pynicotine.__version__
        )
        self.assertIsInstance(local_version, int)

        # Validate version of latest release
        _h_latest_version, latest_version = (
            UpdateChecker.retrieve_latest_version()
        )

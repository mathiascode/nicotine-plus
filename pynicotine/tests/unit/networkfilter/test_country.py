...
class CountryTest(TestCase):
    ...
    def test_ip_country_data_structure(self):
        ...
        self.assertTrue(core.network_filter._countries)
        ...
        self.assertTrue(all(isinstance(item, str) for item in core.network_filter._countries))
        self.assertTrue(all(len(item) == 2 or not item for item in core.network_filter._countries))

        self.assertTrue(all(isinstance(item, int) for item in core.network_filter._ip_range_countries))
        self.assertTrue(all(0 <= item <= 256 for item in core.network_filter._ip_range_countries))

...
BUDDY_SHARES_FOLDER_PATH = os.path.join(
    CURRENT_FOLDER_PATH, ".sharedbuddyfiles"
)
TRUSTED_SHARES_FOLDER_PATH = os.path.join(
    CURRENT_FOLDER_PATH, ".sharedtrustedfiles"
)
INVALID_SHARES_FOLDER_PATH = os.path.join(
    CURRENT_FOLDER_PATH, ".sharedinvalidfiles"
)
AUDIO_FILES = (
...
class SharesTest(TestCase):
    ...
    def setUp(self):
        ...
        config.sections["transfers"]["shared"] = [
            (
                ...
                SHARES_FOLDER_PATH,
                "junk",
            ),  # Superfluous item in tuple
            (
                "invalid",
                os.path.join(SHARES_FOLDER_PATH, "folder2"),
            ),  # Resharing subfolder from previous share
        ]
        config.sections["transfers"]["buddyshared"] = [
            ("Secrets", BUDDY_SHARES_FOLDER_PATH),
            (
                "invalid2",
                os.path.join(BUDDY_SHARES_FOLDER_PATH, "buddies"),
            ),  # Resharing subfolder from previous share
            (
                "invalid3",
                os.path.join(
                    BUDDY_SHARES_FOLDER_PATH, "something2", "nothing2"
                ),
            ),  # File instead of folder
            (
                "Shares",
                INVALID_SHARES_FOLDER_PATH,
        ...
        for folder_path, basename, num_frames in AUDIO_FILES:
            with wave.open(
                os.path.join(folder_path, basename), "wb"
            ) as audio_file:
        ...
        core.shares.load_shares(
            core.shares.share_dbs, core.shares.share_db_paths
        )
    ...
    def test_shares_scan(self):
        ...
        self.assertIn(
            os.path.join(SHARES_FOLDER_PATH, "dummy_file"),
            public_mtimes,
        )
        self.assertIn(
            os.path.join(SHARES_FOLDER_PATH, "audiofile.wav"),
            public_mtimes,
        )
        self.assertIn(
            os.path.join(BUDDY_SHARES_FOLDER_PATH, "audiofile2.wav"),
            buddy_mtimes,
        )
        self.assertIn(
            os.path.join(BUDDY_SHARES_FOLDER_PATH, "something2", "nothing2"),
            buddy_mtimes,
        )
        self.assertIn(
            os.path.join(TRUSTED_SHARES_FOLDER_PATH, "audiofile3.wav"),
            trusted_mtimes,
        )
        self.assertIn(
            os.path.join(
                TRUSTED_SHARES_FOLDER_PATH,
                "folder",
                "folder2",
                "folder3",
                "folder4",
        ...
        self.assertEqual(
            core.shares.share_dbs["public_streams"]["Shares\\folder2"],
            b"\x00\x00\x00\x00",
        )
        self.assertEqual(
            core.shares.share_dbs["buddy_streams"]["Secrets\\folder3"],
            b"\x00\x00\x00\x00",
        )
        self.assertEqual(
            core.shares.share_dbs["trusted_streams"][
                "Trusted\\folder\\folder2"
            ],
            b"\x00\x00\x00\x00",
        )
        ...
        self.assertEqual(
            core.shares.share_dbs["public_files"][
                core.shares.file_path_index[wav_indexes[0]]
            ][0],
            "Shares\\audiofile.wav",
        )
        self.assertEqual(
            core.shares.share_dbs["buddy_files"][
                core.shares.file_path_index[wav_indexes[1]]
            ][0],
            "Secrets\\audiofile2.wav",
        )
        self.assertEqual(
            core.shares.share_dbs["trusted_files"][
                core.shares.file_path_index[wav_indexes[2]]
            ][0],
    ...
    def test_hidden_file_folder_scan(self):
        ...
        self.assertNotIn(
            os.path.join(SHARES_FOLDER_PATH, ".abc", "nothing"),
            public_mtimes,
        )
        self.assertNotIn(
            os.path.join(SHARES_FOLDER_PATH, ".xyz", "nothing"),
            public_mtimes,
        )
        self.assertIn(
            os.path.join(SHARES_FOLDER_PATH, "folder1", "nothing"),
            public_mtimes,
        )
        self.assertIn(
            os.path.join(SHARES_FOLDER_PATH, "folder2", "test", "nothing"),
            public_mtimes,
        )
        self.assertNotIn(
            os.path.join(SHARES_FOLDER_PATH, "folder2", ".poof", "nothing"),
            public_mtimes,
        )
        self.assertIn(
            os.path.join(SHARES_FOLDER_PATH, "folder2", "test", "nothing"),
        ...
        self.assertIn(
            os.path.join(SHARES_FOLDER_PATH, "something", "nothing"),
            public_mtimes,
        )

        self.assertNotIn(
            os.path.join(
                BUDDY_SHARES_FOLDER_PATH,
                "folder3",
                ".poof2",
                "nothing2",
            ),
        ...
        self.assertIn(
            os.path.join(
                BUDDY_SHARES_FOLDER_PATH, "folder3", "test2", "nothing2"
            ),
            buddy_mtimes,
        )
        self.assertNotIn(
            os.path.join(INVALID_SHARES_FOLDER_PATH, "file.txt"),
            buddy_mtimes,
        )

        self.assertNotIn(
            os.path.join(
                TRUSTED_SHARES_FOLDER_PATH, ".hidden_folder", "nothing"
            ),
            trusted_mtimes,
        )
        self.assertIn(
        ...
        self.assertNotIn(
            os.path.join(SHARES_FOLDER_PATH, ".abc_file"), public_files
        )
        self.assertNotIn(
            os.path.join(SHARES_FOLDER_PATH, ".hidden_file"),
            public_files,
        )
        self.assertNotIn(
            os.path.join(SHARES_FOLDER_PATH, ".xyz_file"), public_files
        )
        self.assertIn(
            os.path.join(SHARES_FOLDER_PATH, "dummy_file"), public_files
        )
        self.assertEqual(len(public_files), 5)

        self.assertNotIn(
            os.path.join(BUDDY_SHARES_FOLDER_PATH, ".uvw_file"),
            buddy_files,
        )
        self.assertIn(
            os.path.join(BUDDY_SHARES_FOLDER_PATH, "dummy_file2"),
            buddy_files,
        )
        ...
        self.assertIn(
            os.path.join(TRUSTED_SHARES_FOLDER_PATH, "dummy_file3"),
            trusted_files,
        )

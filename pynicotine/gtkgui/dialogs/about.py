...
Nicotine+ uses the IP2Location LITE database for <a href="https://lite.ip2location.com">IP geolocation</a>.""",
    ...
    def __init__(self, application):
        ...
        for label_widget, text in (
            ...
            (
                self.dependency_versions_label,
                f"GTK {gtk_version}   •   Python {sys.version.split()[0]}",
            ),
            (
                self.website_label,
                (
                    f"<a href='{pynicotine.__website_url__}'"
                    f" title='{pynicotine.__website_url__}'>{website_text}</a>"
                ),
            ),
            (
                self.copyright_label,
                f"<small>{pynicotine.__copyright__}</small>",
            ),
        ):

...
if USE_COLOR_SCHEME_PORTAL:
    ...
    def read_color_scheme():
        ...
        try:
            ...
            log.add_debug(
                "Cannot read color scheme, falling back to GTK theme"
                " preference: %s",
                error,
            )
            return None
    ...
    def on_color_scheme_changed(_proxy, _sender_name, signal_name, parameters):
        ...
        if (
            config.sections["ui"]["dark_mode"]
            or namespace != "org.freedesktop.appearance"
            or name != "color-scheme"
        ):
            return
    ...
    try:
        ...
        log.add_debug(
            "Cannot start color scheme settings portal, falling back to"
            " GTK theme preference: %s",
            portal_error,
        )
...
def set_dark_mode(enabled):
    ...
    if LIBADWAITA_API_VERSION:
        ...
        color_scheme = (
            Adw.ColorScheme.FORCE_DARK if enabled else Adw.ColorScheme.DEFAULT
        )
...
def set_global_css():
    ...
    with open(
        encode_path(os.path.join(css_folder_path, "style.css")), "rb"
    ) as file_handle:
    ...
    if GTK_API_VERSION >= 4:
        ...
        with open(
            encode_path(os.path.join(css_folder_path, "style_gtk4.css")),
            "rb",
        ) as file_handle:
            css.extend(file_handle.read())
        ...
        if sys.platform == "darwin":
            with open(
                encode_path(
                    os.path.join(css_folder_path, "style_gtk4_darwin.css")
                ),
                "rb",
            ) as file_handle:
                css.extend(file_handle.read())
        ...
        if LIBADWAITA_API_VERSION:
            with open(
                encode_path(
                    os.path.join(css_folder_path, "style_libadwaita.css")
                ),
                "rb",
            ) as file_handle:
                css.extend(file_handle.read())
            ...
            if sys.platform in {"win32", "darwin"}:
                with open(
                    encode_path(
                        os.path.join(
                            css_folder_path, "style_libadwaita_csd.css"
                        )
                    ),
                    "rb",
                ) as file_handle:
        ...
        with open(
            encode_path(os.path.join(css_folder_path, "style_gtk3.css")),
            "rb",
        ) as file_handle:
            css.extend(file_handle.read())
...
if GTK_API_VERSION >= 4:
    ICON_THEME = Gtk.IconTheme.get_for_display(  # pylint: disable=no-member
...
def load_custom_icons(update=False):
    ...
    icon_theme_path = os.path.join(
        config.data_folder_path, CUSTOM_ICON_THEME_NAME
    )
    ...
    try:
        ...
        log.add_debug(
            "Failed to remove custom icon theme folder %(theme)s: %(error)s",
    ...
    user_icon_theme_path = os.path.normpath(
        os.path.expandvars(user_icon_theme_path)
    )
...
"""
    ...
    try:
        ...
        with open(
            encode_path(theme_file_path), "w", encoding="utf-8"
        ) as file_handle:
            file_handle.write(theme_file_contents)

    except Exception as error:
        log.add_debug(
            "Failed to enable custom icon theme %(theme)s: %(error)s",
    ...
    for original_name, replacement_name in icon_names:
        for extension in extensions:
            file_path = os.path.join(
                user_icon_theme_path, original_name + extension
            )
...
def _get_custom_font_css():
    ...
    for css_selector, font in (
        ...
        (
            ".transfers-view treeview",
            config.sections["ui"]["transfersfont"],
        ),
        (
            ".userbrowse-view treeview",
            config.sections["ui"]["browserfont"],
        ),
    ):
        font_description = Pango.FontDescription.from_string(font)

        if font_description.get_set_fields() & (
            Pango.FontMask.FAMILY | Pango.FontMask.SIZE
        ):
            font_style = PANGO_STYLES.get(
                font_description.get_style(), "normal"
            )
            font_weight = PANGO_WEIGHTS.get(
                font_description.get_weight(), "normal"
            )
            css.extend(f"""
                {css_selector} {{
...
def _get_custom_color_css():
    ...
    if (
        _is_color_valid(online_color)
        and _is_color_valid(away_color)
        and _is_color_valid(offline_color)
    ):
        css.extend(f"""
            .user-status {{
                -gtk-icon-palette: success {online_color},
                                   warning {away_color},
                                   error {offline_color};
            }}
            """.encode("utf-8"))
    ...
    # Background colors
    for css_selector, color in (("entry", config.sections["ui"]["textbg"]),):
        if _is_color_valid(color):
            css.extend(f"""
                {css_selector} {{
                    background: {color};
...
def update_custom_css():
    ...
    using_custom_icon_theme = (
        GTK_SETTINGS.props.gtk_icon_theme_name == CUSTOM_ICON_THEME_NAME
    )
    icon_style = "regular" if using_custom_icon_theme else "symbolic"
    css = bytearray(f"""
        .colored-icon {{
            -gtk-icon-style: {icon_style};
        }}
        """.encode("utf-8"))
...
def update_tag_visuals(tag, color_id):
    ...
    if (
        color_id == "urlcolor"
        and tag_props.underline != Pango.Underline.SINGLE
    ):
    ...
    weight_style = (
        Pango.Weight.BOLD if username_style == "bold" else Pango.Weight.NORMAL
    )
    ...
    underline_style = (
        ...
        if username_style == "underline"
        else Pango.Underline.NONE
    )

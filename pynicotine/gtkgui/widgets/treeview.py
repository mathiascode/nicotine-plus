...
# Michael Labouebe <gfarmerfr@free.fr> COPYRIGHT (C) 2008-2009 quinox
...
class TreeView:
    ...
    def __init__(
        ...
        delete_accelerator_callback=None,
        search_entry=None,
    ):

        self.window = window
        self.widget = Gtk.TreeView(
            fixed_height_mode=True, has_tooltip=True, visible=True
        )
        self.model = None
        ...
        self.notify_value_handler = self._v_adjustment.connect(
            "notify::value", self.on_v_adjustment_value
        )

        if GTK_API_VERSION >= 4:
            # pylint: disable=no-member
            parent.set_child(self.widget)
        else:
            # pylint: disable=no-member
        ...
        if activate_row_callback:
            self.widget.connect(
                "row-activated",
                self.on_activate_row,
                activate_row_callback,
            )
        ...
        if focus_in_callback:
            if GTK_API_VERSION >= 4:
                ...
                focus_controller.connect(
                    "enter", self.on_focus_in, focus_in_callback
                )
        ...
        if select_row_callback:
            self._selection.connect(
                "changed", self.on_select_row, select_row_callback
            )
        ...
        if delete_accelerator_callback:
            Accelerator(
                "Delete",
                self.widget,
                self.on_delete_accelerator,
                delete_accelerator_callback,
            )

        if search_entry:
            self.widget.set_search_entry(search_entry)

        self._query_tooltip_handler = self.widget.connect(
    ...
    def create_model(self):
        ...
        # Bypass Tree/ListStore overrides for improved performance in
        # set_value()
        gtk_module = gi.module.get_introspection_module("Gtk")
        model_class = (
            gtk_module.TreeStore if self.has_tree else gtk_module.ListStore
        )

        if hasattr(gtk_module.ListStore, "insert_with_valuesv"):
            gtk_module.ListStore.insert_with_values = (
                gtk_module.ListStore.insert_with_valuesv
            )
    ...
    def _append_columns(self, cols, column_config):
        ...
        for column_id, column in cols.items():
            column.set_visible(
                bool(column_config.get(column_id, {}).get("visible", True))
            )
    ...
    def _initialise_column_ids(self, columns):
        ...
        for column_index, (column_id, column_data) in enumerate(
            columns.items()
        ):
    ...
    def _initialise_columns(self, columns):
        ...
        for column_index, (column_id, column_data) in enumerate(
            ...
            if default_sort_type:
                ...
                self._default_sort_type = (
                    Gtk.SortType.DESCENDING
                    if default_sort_type == "descending"
            ...
            if self._widget_name:
                ...
                # Restore saved column width if the column size is fixed. For
                # expandable columns, the width becomes the minimum width, so
                # use the default value in those cases.
                ...
                if column_sort_type and self._persistent_sort:
                    ...
                    self._sort_type = (
                        Gtk.SortType.DESCENDING
                        if column_sort_type == "descending"
                        else Gtk.SortType.ASCENDING
                    )
                    self.model.set_sort_column_id(
                        self._sort_column, self._sort_type
                    )
            ...
            mode = (
                ...
                if len(columns) > 1
                else Gtk.CellRendererMode.INERT
            )
            xalign = 0.0
            ...
            if column_type == "text":
                ...
                text_underline_column = column_data.get(
                    "text_underline_column"
                )
                ...
                if text_underline_column:
                    column.add_attribute(
                        renderer,
                        "underline",
                        self._column_ids[text_underline_column],
                    )
                ...
                if text_weight_column:
                    column.add_attribute(
                        renderer,
                        "weight",
                        self._column_ids[text_weight_column],
                    )
                ...
                renderer = Gtk.CellRendererProgress(
                    mode=mode, ypad=progress_padding
                )
                column = Gtk.TreeViewColumn(
                    title=title,
                    cell_renderer=renderer,
                ...
                renderer = Gtk.CellRendererToggle(
                    mode=mode, xalign=xalign, xpad=13
                )
                renderer.connect(
                    "toggled",
                    self.on_toggle,
                    column_data["toggle_callback"],
                )
                ...
                renderer = Gtk.CellRendererPixbuf(
                    mode=mode, xalign=1.0, **icon_args
                )
                column = Gtk.TreeViewColumn(
                    title=title,
                    cell_renderer=renderer,
                    icon_name=column_index,
                )
            ...
            if GTK_API_VERSION >= 4:
                ...
                column_header.add_controller(gesture_click)
            else:
            ...
            title_widget = (
                next(iter(title_container))
                if xalign < 1
                else list(title_container)[-1]
            )
            ...
            if xalign == 1 and GTK_API_VERSION >= 4:
                # Gtk.TreeViewColumn.set_alignment() only changes the sort
                # arrow position in GTK 4 Actually align the label to the right
                # here instead
            ...
            if sensitive_column:
                column.add_attribute(
                    renderer,
                    "sensitive",
                    self._column_ids[sensitive_column],
                )
            ...
            if self._widget_name:
                column.connect(
                    "notify::x-offset", self.on_column_position_changed
                )
        ...
        self._columns_changed_handler = self.widget.connect(
            "columns-changed", self._update_column_properties
        )
    ...
    def save_columns(self):
        ...
        for column in self.widget.get_columns():
            ...
            # A column width of zero should not be saved to the config. When a
            # column is hidden, the correct width will be remembered during the
            # run it was hidden. Subsequent runs will yield a zero width, so we
            # attempt to re-use a previously saved non-zero column width
            # instead.
            try:
                if width <= 0:
                    if not visible:
                        saved_columns[title] = {
                            "visible": visible,
                            "width": column_config[self._widget_name][title][
                                "width"
                            ],
                        }
            ...
            if (
                sort_column_id == self._sort_column
                and sort_column_id != self._default_sort_column
            ):
                columns["sort"] = (
                    "descending"
    ...
    def add_row(
        ...
        if self.has_tree:
            ...
            self.iterators[key] = iterator = self.model.insert_with_values(
                position, self._column_numbers, values
            )
    ...
    def on_column_header_pressed(
        ...
        if self._data_types[sort_column_id] == str or column_id in {
            "in_queue",
            "queue_position",
        }:
            # String value (or queue position column): ascending sort by
            # default
    ...
    def on_column_header_menu(self, menu, _treeview):
        ...
        columns = self.widget.get_columns()
        visible_columns = [
            column for column in columns if column.get_visible()
        ]
        menu.clear()
        ...
        for column_num, column in enumerate(columns, start=1):
            ...
            menu.actions[title].set_state(
                GLib.Variant("b", column in visible_columns)
            )

            if column in visible_columns:
                menu.actions[title].set_enabled(len(visible_columns) > 1)

            menu.actions[title].connect(
                "activate", self.on_column_header_toggled, column
            )
    ...
    def on_v_adjustment_value(self, *_args):
        ...
        if (
            not self._is_scrolling_to_row
            and upper != self._v_adjustment_upper
            and self._v_adjustment_value <= 0
        ):
    ...
    def on_tooltip(self, _widget, pos_x, pos_y, _keyboard_mode, tooltip):
        ...
        # is_blank_at_pos() crashes in 4.12.0 - 4.12.2 Remove check when GTK 4
        # version in Snap package is >= 4.12.3
        if (
            (4, 12, 2)
            >= (GTK_API_VERSION, GTK_MINOR_VERSION, GTK_MICRO_VERSION)
            >= (4, 12, 0)
        ):
            ...
            is_blank, path, column, _cell_x, _cell_y = (
                self.widget.is_blank_at_pos(bin_x, bin_y)
            )
    ...
    def on_copy_cell_data_accelerator(self, *_args):
        ...
        iterator = self.model.get_iter(path)
        value = str(
            self.model.get_value(iterator, column.get_sort_column_id())
        )
        ...
        if column.type == "icon":
            value = self.get_icon_label(
                column, value, is_short_country_label=True
            )
...
def create_grouping_menu(window, active_mode, callback):
    ...
    menuitem = Gio.MenuItem.new(
        _("Group by Folder"), f"win.{action_id}::folder_grouping"
    )
    menu.append_item(menuitem)

    menuitem = Gio.MenuItem.new(
        _("Group by User"), f"win.{action_id}::user_grouping"
    )
    menu.append_item(menuitem)
...
def set_treeview_selected_row(treeview, bin_x, bin_y):
    ...
    if pathinfo is not None:
        ...
        # Make sure we don't attempt to select a single row if the row is
        # already in a selection of multiple rows, otherwise the other rows
        # will be unselected
        if (
            selection.count_selected_rows() <= 1
            or not selection.path_is_selected(path)
        ):
            treeview.grab_focus()

...
def load(scope, path):
    ...
    if path not in ui_data:
        with open(
            encode_path(os.path.join(GTK_GUI_FOLDER_PATH, "ui", path)),
            encoding="utf-8",
        ) as file_handle:
            ...
            while tag_start_pos > -1:
                ...
                if original_string != translated_string:
                    ui_content = (
                        ui_content[:string_start_pos]
                        + translated_string
                        + ui_content[string_end_pos:]
                    )

                # Find next translatable string
                new_string_end_pos = string_end_pos + (
                    len(translated_string) - len(original_string)
                )
            ...
            if GTK_API_VERSION >= 4:
                ...
                if GTK_MINOR_VERSION >= 10:
                    ui_content = ui_content.replace(
                        "GtkColorButton", "GtkColorDialogButton"

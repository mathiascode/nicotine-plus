...
class FileChooser:
    ...
    active_chooser = (
        None  # Class variable keeping the file chooser object alive
    )
    ...
    def __init__(
        ...
        if initial_folder:
            initial_folder = os.path.normpath(
                os.path.expandvars(initial_folder)
            )
        ...
        try:
            ...
            if initial_folder:
                self.file_chooser.set_initial_folder(
                    Gio.File.new_for_path(initial_folder)
                )
    ...
    def on_finish(self, _dialog, result):
        ...
        if self.select_multiple:
            selected = [
                os.path.normpath(i.get_path()) for i in selected_result
            ]
        else:
    ...
    def on_response(self, _dialog, response_id):
        ...
        if self.select_multiple:
            selected = [
                os.path.normpath(i.get_path())
                for i in self.file_chooser.get_files()
            ]
        else:
            selected_file = self.file_chooser.get_file()
            selected = (
                os.path.normpath(selected_file.get_path())
                if selected_file
                else None
            )
...
class FolderChooser(FileChooser):
    ...
    def __init__(
        ...
        if select_multiple:
            ...
            self.finish_method = (
                self.file_chooser.select_multiple_folders_finish
            )
            return
...
class ImageChooser(FileChooser):
    ...
    def __init__(
        ...
        if GTK_API_VERSION == 3 and sys.platform not in {
            "win32",
            "darwin",
        }:
            # Image preview
            self.file_chooser.connect(
                "update-preview", self.on_update_image_preview
            )
    ...
    def on_update_image_preview(self, chooser):
        ...
        try:
            image_data = GdkPixbuf.Pixbuf.new_from_file_at_size(
                file_path, width=300, height=700
            )
...
class FileChooserSave(FileChooser):
    ...
    def __init__(
        ...
        super().__init__(
            parent, callback, callback_data, title, initial_folder
        )
...
class FileChooserButton:
    ...
    def __init__(
        ...
        self.chooser_button = Gtk.Button(
            hexpand=True, valign=Gtk.Align.CENTER, visible=True
        )
        ...
        self.icon = Gtk.Image(
            icon_name=icon_names.get(chooser_type), visible=True
        )
        self.label = Gtk.Label(
            label=_("(None)"),
            ellipsize=Pango.EllipsizeMode.END,
            width_chars=3,
            mnemonic_widget=self.chooser_button,
            xalign=0,
            visible=True,
        )
        ...
        if GTK_API_VERSION >= 4:
            ...
            widget.append(self.chooser_button)
            widget.append(self.open_folder_button)
            label_container.append(self.icon)
            label_container.append(self.label)
            self.chooser_button.set_child(label_container)

            self.open_folder_button.set_icon_name("folder-open-symbolic")

            if end_button:
                widget.append(end_button)
        else:
            container.add(widget)
            widget.add(self.chooser_button)
            widget.add(self.open_folder_button)
            label_container.add(self.icon)
            label_container.add(self.label)
            self.chooser_button.add(label_container)

            self.open_folder_button.set_image(
                Gtk.Image(icon_name="folder-open-symbolic")
            )

            if end_button:
                widget.add(end_button)
    ...
    def on_open_file_chooser_response(self, selected, _data):
        ...
        if selected.startswith(config.data_folder_path):
            ...
            selected = selected.replace(
                config.data_folder_path, "${NICOTINE_DATA_HOME}", 1
            )
    ...
    def on_open_folder(self, *_args):
        ...
        folder_path = os.path.expandvars(
            path if self.chooser_type == "folder" else os.path.dirname(path)
        )
    ...
    def set_path(self, path):
        ...
        self.chooser_button.set_tooltip_text(
            os.path.expandvars(path)
        )  # Show path without env variables

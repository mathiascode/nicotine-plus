...
class TextView:
    ...
    try:
        ...
        log.add_debug(
            "Cannot load cursors from theme, falling back to default cursor"
        )
        DEFAULT_CURSOR = POINTER_CURSOR = TEXT_CURSOR = None

    MAX_NUM_LINES = 50000
    URL_REGEX = re.compile(
        "(\\w+\\://[^\\s]+)|(www\\.\\w+\\.[^\\s]+)|(mailto\\:[^\\s]+)"
    )
    ...
    def __init__(
        ...
        self.notify_upper_handler = self.adjustment.connect(
            "notify::upper", self.on_adjustment_upper_changed
        )
        self.notify_value_handler = self.adjustment.connect(
            "notify::value", self.on_adjustment_value_changed
        )
        ...
        if GTK_API_VERSION >= 4:
            ...
            self.gesture_click_primary = Gtk.GestureMultiPress(
                widget=scrollable_container
            )
            self.gesture_click_secondary = Gtk.GestureMultiPress(
                widget=scrollable_container
            )
            ...
            self.widget.connect(
                "motion-notify-event", self.on_move_cursor_event
            )

        self.gesture_click_primary.set_propagation_phase(
            Gtk.PropagationPhase.CAPTURE
        )
        self.gesture_click_primary.connect(
            "released", self.on_released_primary
        )

        self.gesture_click_secondary.set_propagation_phase(
            Gtk.PropagationPhase.CAPTURE
        )
        self.gesture_click_secondary.set_button(Gdk.BUTTON_SECONDARY)
        self.gesture_click_secondary.connect(
            "pressed", self.on_pressed_secondary
        )
    ...
    def scroll_bottom(self):
        self.adjustment_value = (
            self.adjustment.get_upper() - self.adjustment.get_page_size()
        )
    ...
    def append_line(
        ...
        if self.textbuffer.get_char_count() > 0:
            # No tag applied on line breaks to prevent visual glitch where text
            # on the next line has the wrong color
            self._insert_text("\n")

        # Tag usernames with popup menu creating tag, and away/online/offline
        # colors
        ...
        if self.parse_urls and (
            "://" in line or "www." in line or "mailto:" in line
        ):
            # Match first url
    ...
    def get_text(self):
        ...
        return self.textbuffer.get_text(
            start_iter, self.end_iter, include_hidden_chars=True
        )

    def get_tags_for_pos(self, pos_x, pos_y):

        buf_x, buf_y = self.widget.window_to_buffer_coords(
            Gtk.TextWindowType.WIDGET, pos_x, pos_y
        )
        over_text, iterator, _trailing = self.widget.get_iter_at_position(
            buf_x, buf_y
        )

        if not over_text:
            # Iterators are returned for whitespace after the last character,
            # avoid accidental URL clicks
            return []
    ...
    def set_text(self, text):
        """Sets text without any additional processing, and clears the undo
        stack."""
    ...
    def update_cursor(self, pos_x, pos_y):
        ...
        if self.cursor_window is None:
            # pylint: disable=no-member
            self.cursor_window = self.widget.get_window(
    ...
    def create_tag(
        self, color_id=None, callback=None, username=None, url=None
    ):
    ...
    def on_adjustment_upper_changed(self, *_args):
        ...
        new_adjustment_bottom = (
            self.adjustment.get_upper() - self.adjustment.get_page_size()
        )

        if (
            self.auto_scroll
            and self.adjustment_value >= self.adjustment_bottom
        ):
    ...
    def on_adjustment_value_changed(self, *_args):
        ...
        if new_value.is_integer() and (0 < new_value < self.adjustment_bottom):
            # The textview scrolls up on its own sometimes. Ignore these
            # garbage values.
            return
...
class ChatView(TextView):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if status_users is not None:
            # In chatrooms, we only want to set the online status for users
            # that are currently in the room, even though we might know their
            # global status
    ...
    def append_log_lines(
        self, folder_path, basename, num_lines, timestamp_format
    ):
            ...
            if " [" in line and "] " in line:
                ...
                if end > start:
                    ...
                    if user == login:
                        ...
                        login
                        and find_whole_word(login.lower(), text.lower()) > -1
                    ):
                        message_type = "hilite"
    ...
    def get_user_tag(self, username):
        ...
        if username not in self.user_tags:
            self.user_tags[username] = self.create_tag(
                callback=self.username_event, username=username
            )

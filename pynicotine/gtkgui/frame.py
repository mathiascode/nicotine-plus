...
class NicotineFrame(UserInterface):
    ...
    def on_switch_page(self, _notebook, page, _page_num):
        ...
        if page == self.chatrooms_page:
            curr_page = self.chatrooms.get_current_page()
            curr_page_num = self.chatrooms.get_current_page_num()

            if curr_page is not None:
                self.chatrooms.widget.emit("switch-page", curr_page, curr_page_num)
            else:
                GLib.idle_add(lambda: self.chatrooms_entry.grab_focus() == -1)

        elif page == self.private_page:
            curr_page = self.privatechat.get_current_page()
            curr_page_num = self.privatechat.get_current_page_num()

            if curr_page is not None:
                self.privatechat.widget.emit("switch-page", curr_page, curr_page_num)
            else:
            ...
            curr_page = self.search.get_current_page()
            curr_page_num = self.search.get_current_page_num()

            if curr_page is not None:
                self.search.widget.emit("switch-page", curr_page, curr_page_num)

            GLib.idle_add(lambda: self.search_entry.grab_focus() == -1)

        elif page == self.userinfo_page:
            curr_page = self.userinfo.get_current_page()
            curr_page_num = self.userinfo.get_current_page_num()

            if curr_page is not None:
                self.userinfo.widget.emit("switch-page", curr_page, curr_page_num)
            else:
                GLib.idle_add(lambda: self.userinfo_entry.grab_focus() == -1)

        elif page == self.userbrowse_page:
            curr_page = self.userbrowse.get_current_page()
            curr_page_num = self.userbrowse.get_current_page_num()

            if curr_page is not None:
                self.userbrowse.widget.emit("switch-page", curr_page, curr_page_num)
            else:
                GLib.idle_add(lambda: self.userbrowse_entry.grab_focus() == -1)

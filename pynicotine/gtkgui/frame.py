...
class NicotineFrame(UserInterface):
    ...
    def __init__(self, application, network_processor, use_trayicon, start_hidden, bindip, port, ci_mode):
        ...
        setup_accelerator("<Primary>w", self.MainWindow, self.on_tab_close)
        setup_accelerator("<Primary>F4", self.MainWindow, self.on_tab_close)

        setup_accelerator("<Primary>Tab", self.MainWindow, self.on_tab_cycle)
        setup_accelerator("<Primary><Shift>Tab", self.MainWindow, self.on_tab_cycle, True)

        for num in range(10):
            setup_accelerator("<Primary>" + str(num), self.MainWindow, self.on_change_primary_tab, num)
            setup_accelerator("<Alt>" + str(num), self.MainWindow, self.on_change_primary_tab, num)

...
class UserList(UserInterface):
    ...
    def __init__(self, frame, core):
        ...
        self.list_view = TreeView(
            ...
            columns=[
                # Visible columns
                {"column_id": "status", "column_type": "icon_data", "title": _("Status"), "width": 25,
                 "sort_column": 10, "hide_header": True},
                {"column_id": "country", "column_type": "icon_name", "title": _("Country"), "width": 25,
    ...
    def add_user(self, user, row):
        ...
        self.user_iterators[user] = self.list_view.add_row([
            get_status_icon(0), get_flag_icon_name(country), user, empty_str, empty_str, trusted, notify,
            prioritized, last_seen, note, empty_int, empty_int, empty_int, time_from_epoch, country]
        )
    ...
    def server_disconnect(self):
        ...
        for iterator in self.list_view.get_all_rows():
            self.list_view.set_row_value(iterator, 0, get_status_icon(0))
            self.list_view.set_row_value(iterator, 3, empty_str)
            self.list_view.set_row_value(iterator, 4, empty_str)
            self.list_view.set_row_value(iterator, 10, empty_int)
            self.list_view.set_row_value(iterator, 11, empty_int)
            self.list_view.set_row_value(iterator, 12, empty_int)

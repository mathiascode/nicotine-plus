...
class RoomList(Popover):
    ...
    def __init__(self, window):
        ...
        for toggle in (
            ...
            parent.gesture_click.connect(
                "released", self.on_toggle_label_pressed, toggle
            )

        self.private_room_toggle.set_active(
            config.sections["server"]["private_chatrooms"]
        )
        self.private_room_toggle.connect(
            "notify::active", self.on_toggle_accept_private_room
        )

        Accelerator("<Primary>f", self.widget, self.on_search_accelerator)
        self.completion_entry = CompletionEntry(
            window.chatrooms_entry, self.list_view.model, column=0
        )
    ...
    def add_room(self, room, user_count=0, is_private=False, is_owned=False):
        ...
        text_underline = (
            Pango.Underline.SINGLE if is_owned else Pango.Underline.NONE
        )
    ...
    def on_popup_menu(self, menu, _widget):
        ...
        menu.actions[_("Join Room")].set_enabled(
            room not in core.chatrooms.joined_rooms
        )
        menu.actions[_("Leave Room")].set_enabled(
            room in core.chatrooms.joined_rooms
        )

        menu.actions[_("Disown Private Room")].set_enabled(
            is_private_room_owned
        )
        menu.actions[_("Cancel Room Membership")].set_enabled(
            is_private_room_member and not is_private_room_owned
        )
    ...
    def on_toggle_accept_private_room(self, *_args):
        core.chatrooms.request_private_room_toggle(
            self.private_room_toggle.get_active()
        )

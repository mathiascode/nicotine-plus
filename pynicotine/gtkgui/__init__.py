...
def get_default_gtk_version():
    ...
    try:
        ...
        try:
            ...
            # If screen reader is enabled, use GTK 3 until treeviews have been
            # ported to Gtk.ColumnView. Gtk.TreeView doesn't support screen
            # readers in GTK 4.
            if dbus_proxy.Get("(ss)", "org.a11y.Status", "IsEnabled"):
                log.add_debug(
                    "Screen reader enabled, using GTK 3 for improved"
                    " accessibility"
                )
                return "3"
...
def check_gtk_version(gtk_api_version):
    ...
    try:
        ...
        return _("Cannot find %s, please install it.") % (
            "PyGObject >=" + ".".join(str(x) for x in pygobject_version)
        )
    ...
    try:
        ...
        return (
            _("Cannot find %s, please install it.")
            % f"GTK >={gtk_api_version}"
        )

    from gi.repository import Gtk

    if sys.platform == "win32":
        # noqa: F401 # pylint:disable=no-name-in-module,unused-import
    ...
    log.add(
        ...
        {"program": "GTK", "version": gtk_version},
    )
...
def run(hidden, ci_mode, multi_instance):
    ...
    if getattr(sys, "frozen", False):
        ...
        if sys.platform == "win32":
            ...
            ctypes.windll.kernel32.SetDllDirectoryW(
                os.path.join(executable_folder, "lib")
            )

        elif sys.platform == "darwin":
            resources_folder = os.path.abspath(
                os.path.join(executable_folder, "..", "Resources")
            )

        os.environ["XDG_DATA_DIRS"] = os.path.join(resources_folder, "share")
        os.environ["FONTCONFIG_FILE"] = os.path.join(
            resources_folder, "share", "fonts", "fonts.conf"
        )
        os.environ["FONTCONFIG_PATH"] = os.path.join(
            resources_folder, "share", "fonts"
        )
        os.environ["GDK_PIXBUF_MODULE_FILE"] = os.path.join(
            executable_folder, "lib", "pixbuf-loaders.cache"
        )
        os.environ["GI_TYPELIB_PATH"] = os.path.join(
            executable_folder, "lib", "typelibs"
        )
        os.environ["GSETTINGS_SCHEMA_DIR"] = os.path.join(
            executable_folder, "lib", "schemas"
        )

    if sys.platform == "win32":
        # 'win32' PangoCairo backend on Windows is too slow, use 'fontconfig'
        # instead
        ...
        # Use Cairo software rendering due to flickering issues in the GPU
        # renderer (#2859). Reevaluate when the new GPU renderers are stable:
    ...
    if os.environ.get("GSK_RENDERER") == "gl":
        ...
        os.environ["GDK_DEBUG"] = ",".join(
            ("gl-no-fractional", os.environ.get("GDK_DEBUG", ""))
        )

    error = check_gtk_version(
        gtk_api_version=os.environ.get(
            "NICOTINE_GTK_VERSION", get_default_gtk_version()
        )
    )
    ...
    if not ci_mode and Gdk.Display.get_default() is None:
        log.add(
            _(
                "No graphical environment available, using headless (no GUI)"
                " mode"
            )
        )
        return None

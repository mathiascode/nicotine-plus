...
def check_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=_("Website: %s") % __website_url__,
        add_help=False,
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"{__application_name__} {__version__}",
        help=_("display version and exit"),
    )

    # Disables critical error dialog; used for integration tests
    parser.add_argument(
        "--ci-mode", action="store_true", help=argparse.SUPPRESS
    )
    ...
    if args.config:
        ...
        # Since a custom config was specified, allow another instance of the
        # application to open
        multi_instance = True
...
def check_python_version():
    ...
    if sys.version_info < python_version:
        return _(
            ...
            " (%(old_version)s). You should install Python"
            " %(min_version)s or newer."
        ) % {
...
def set_up_python():
    ...
    if is_frozen:
        ...
        os.environ["SSL_CERT_FILE"] = os.path.join(
            executable_folder, "lib/cert.pem"
        )
...
def rescan_shares():
    ...
    if not core.shares.rescan_shares(use_thread=False):
        ...
        log.add(
            _(
                "Failed to scan shares. Please close other Nicotine+"
...
def run():
    ...
    if not os.path.isdir(LOCALE_PATH):
        log.add(
            "Translation files (.mo) are unavailable, using default English"
            " strings"
        )

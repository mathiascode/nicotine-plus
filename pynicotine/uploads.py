...
class Uploads(Transfers):
    __slots__ = ("pending_shutdown", "upload_speed", "token", "_queue_positions",
                 "_queue_position_users", "_fifo_position_requested", "_pending_network_msgs",
    ...
    def __init__(self):
        ...
        self._fifo_position_requested = False
    ...
    def _place_in_queue_request(self, msg):
        ...
        if is_fifo_queue:
            if is_fifo_queue != self._fifo_position_requested or upload not in self._queue_positions:
                self._queue_position_users.clear()
                ...
                if is_privileged_queue:
                    ...
                    self._queue_positions = {i_upload: i for i, i_upload in enumerate(self.queued_transfers)}
            ...
            if upload not in user_queue_positions:
                ...
                user_queue_positions.update({
                    i_upload: i for i, i_upload in enumerate(self.queued_users[username].values())
                })

            position = user_queue_positions[upload]
        ...
        self._fifo_position_requested = is_fifo_queue

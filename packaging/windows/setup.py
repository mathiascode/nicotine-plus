# !/usr/bin/env python3 COPYRIGHT (C) 2021-2024 Nicotine+ Contributors
#
...
USE_LIBADWAITA = (
    GTK_VERSION == "4" and os.environ.get("NICOTINE_LIBADWAITA") == "1"
)
...
def process_files(
    ...
    for full_path in glob.glob(
        os.path.join(folder_path, "**"), recursive=recursive
    ):
        short_folder_path = os.path.dirname(
            os.path.relpath(full_path, folder_path)
        )
        real_full_path = os.path.realpath(full_path)
        short_path = os.path.join(
            short_folder_path, os.path.basename(real_full_path)
        )
...
def add_pixbuf_loaders():
    ...
    with open(
        temp_loaders_file, "w", encoding="utf-8"
    ) as temp_file_handle, open(
        os.path.join(SYS_BASE_PATH, loaders_file), "r", encoding="utf-8"
    ) as real_file_handle:
        data = real_file_handle.read()

        if sys.platform == "win32":
            data = data.replace(
                "lib\\\\gdk-pixbuf-2.0\\\\2.10.0\\\\loaders", "lib"
            )

        elif sys.platform == "darwin":
            data = data.replace(
                os.path.join(
                    SYS_BASE_PATH, "lib/gdk-pixbuf-2.0/2.10.0/loaders"
                ),
                "@executable_path/lib",
            )
    ...
    add_file(
        file_path=temp_loaders_file,
        output_path="lib/pixbuf-loaders.cache",
    )
    add_files(
...
def _add_typelibs_callback(full_path, short_path, _callback_data=None):
    ...
    temp_file_typelib = os.path.join(
        TEMP_PATH, short_path.replace(".gir", ".typelib")
    )
    ...
    with open(temp_file_gir, "w", encoding="utf-8") as temp_file_handle, open(
        ...
        data = real_file_handle.read()
        data = data.replace(
            'shared-library="lib', 'shared-library="@loader_path/lib'
        )
        temp_file_handle.write(data)

    subprocess.check_call([
        "g-ir-compiler",
        f"--output={temp_file_typelib}",
        temp_file_gir,
    ])
...
def add_typelibs():
    ...
    if sys.platform == "darwin":
        ...
        process_files(
            ...
            callback=_add_typelibs_callback,
            starts_with=required_typelibs,
            ends_with=".gir",
        )
...
def add_gtk():
    ...
    if sys.platform == "win32":
        ...
        add_file(
            file_path=os.path.join(LIB_PATH, "gdbus.exe"),
            output_path="gdbus.exe",
        )
    ...
    add_file(
        file_path=os.path.join(
            SYS_BASE_PATH, "share/glib-2.0/schemas/gschemas.compiled"
        ),
    ...
    # Fontconfig
    add_files(
        folder_path=os.path.join(SYS_BASE_PATH, "etc/fonts"),
        output_path="share/fonts",
        ends_with=".conf",
        recursive=True,
    )
...
def add_icon_packs():
    ...
    required_icon_packs = ("Adwaita",)
    add_files(
        folder_path=os.path.join(SYS_BASE_PATH, "share/icons"),
        output_path="share/icons",
        starts_with=required_icon_packs,
        ends_with=(".theme", ".svg"),
        recursive=True,
    )
...
def add_themes():
    ...
    add_files(
        folder_path=os.path.join(SYS_BASE_PATH, "share/themes"),
        output_path="share/themes",
        starts_with=required_themes,
        ends_with=".css",
        recursive=True,
    )
...
def add_translations():
    ...
    # noqa: E402 # pylint: disable=import-self,no-name-in-module
    from setup import build_translations

    build_translations()

    add_files(
        folder_path=os.path.join(SYS_BASE_PATH, "share/locale"),
        output_path="share/locale",
        starts_with=tuple(i[0] for i in pynicotine.i18n.LANGUAGES),
        ends_with=f"gtk{GTK_VERSION}0.mo",
        recursive=True,
    )
    ...
    options={
        ...
        "build_exe": {
            "build_exe": os.path.join(
                BUILD_PATH, "package", pynicotine.__application_name__
            ),
        ...
        "bdist_mac": {
            ...
            "codesign_entitlements": os.path.join(
                CURRENT_PATH, "codesign-entitlements.plist"
            ),
        ...
        "bdist_dmg": {"applications_shortcut": True},
    },
    data_files=[],
    packages=[],
    executables=[

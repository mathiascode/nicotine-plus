...
def generate_sha256_hashes():
    """Generates SHA256 checksums for files placed in the same folder as this
    script."""
    ...
    for entry in os.scandir(current_folder_path):
        ...
        with open(
            entry.path + ".sha256", "w", encoding="utf-8"
        ) as file_handle:
            output = (
                sha256_hash.hexdigest() + "  " + os.path.basename(entry.name)
            )
